...
const Canvas: React.FC = () => {
  ...
  const onDesignPage = (item: NodeItem) => {
    const { id } = item
    //window.open(`/component-process-designer/pageDesigner?ntype=page&appId=${appId}&xwPageId=${id}&subPageSecond=true`, '_blank');
    //跳转到新版页面编辑器
    window.open(`/page-designer/index.html?appId=${appId}&pageId=${id}`, '_blank');
  }
  ...
  const onFlowDesign = async (item: NodeItem) => {
    ...
    window.open(`/flow/manage/designer?id=${flowKey}`, '_blank');
  }
